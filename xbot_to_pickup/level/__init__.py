...
class ContentTypeRemover(urllib2.BaseHandler):
...
def urlopen(url,p):
	...
	#t.add_header('X-FORWARDED-FOR', ".".join(str(random.randint(0, 255)) for _ in range(4)) )
	#t.add_header('CLIENT-IP', ".".join(str(random.randint(0, 255)) for _ in range(4)) )
	return urllib2.build_opener(ContentTypeRemover()).open(t)
...
class Level:
	...
	def saveLevel(self,description=""):
		man = CCManager.newManager()
		man.injectLevel(self.__str__(),levelName=self.name,levelDesc=description)
		man.save()
def getSong(lid):
